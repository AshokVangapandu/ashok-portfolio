...
console.log('Tracing send-testimonial-email (APPROVED) Edge Function pipeline on Production...\n');

const payload = JSON.stringify({
  record: {
    id: 'test-approved-testimonial-diagnostic-id',
    ...
    rating: 5,
    testimonial: 'This is a diagnostic verification that the approval email fires and dispatches correctly.',
    consent_public: true,
    status: 'approved',
  ...
  res.on('end', () => {
    ...
    try {
      ...
      if (json.error && json.error.includes('Unauthorized')) {
        ...
        console.log('SUCCESS: Approval email webhook executed successfully!');
      }
    } catch (e) {
