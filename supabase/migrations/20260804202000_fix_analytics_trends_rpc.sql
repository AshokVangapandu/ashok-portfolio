-- Fix nested aggregates in get_analytics_trends
...
BEGIN
  ...
  IF trend_mode = 'weekly' THEN
    ...
    FROM (
      ...
      FROM (
        SELECT generate_series(st, nt, interval '1 week') as week_date
      ) w
      LEFT JOIN public.visitor_sessions v ON date_trunc('week', v.created_at) = date_trunc('week', w.week_date)
  ...
  ELSIF trend_mode = 'monthly' THEN
    ...
    FROM (
      SELECT 
        ...
        COALESCE(COUNT(v.id), 0)::int as visitors
      FROM (
        SELECT generate_series(st, nt, interval '1 month') as month_date
      ) m
      LEFT JOIN public.visitor_sessions v ON date_trunc('month', v.created_at) = date_trunc('month', m.month_date)
    ...
    IF range_filter = 'today' THEN
      ...
      FROM (
        SELECT 
          ...
          COALESCE(COUNT(v.id), 0)::int as visitors
        FROM (
          SELECT generate_series(st, nt, interval '1 hour') as hour_date
        ) h
        LEFT JOIN public.visitor_sessions v ON date_trunc('hour', v.created_at) = date_trunc('hour', h.hour_date)
    ...
    ELSE
      ...
      FROM (
        SELECT 
          ...
          COALESCE(COUNT(v.id), 0)::int as visitors
        FROM (
          SELECT generate_series(st, nt, interval '1 day') as day_date
        ) d
        LEFT JOIN public.visitor_sessions v ON v.created_at::date = d.day_date::date
        GROUP BY d.day_date
        ORDER BY d.day_date
      ) t;
    END IF;
