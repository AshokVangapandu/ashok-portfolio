      ...
      if (mode === 'maintenance') {
        ...
        const userContent = currentUser ? `
          <div style="display:flex; flex-direction:column; align-items:center; gap:14px; width:100%; box-sizing:border-box;">
            <div style="display:flex; align-items:center; gap:12px; width:100%;">
              ${currentUser.user_metadata?.avatar_url || currentUser.user_metadata?.picture ? `<img src="${currentUser.user_metadata.avatar_url || currentUser.user_metadata.picture}" referrerpolicy="no-referrer" style="width:38px; height:38px; border-radius:50%; border:2px solid #7C3AED; flex-shrink:0;" />` : `<div style="width:38px; height:38px; border-radius:50%; background:#7C3AED; color:#FFF; display:flex; align-items:center; justify-content:center; font-weight:700; font-size:15px; flex-shrink:0;">${(currentUser.user_metadata?.full_name || currentUser.email || 'U').charAt(0).toUpperCase()}</div>`}
        ...
        const userContent = currentUser ? `
          <div style="display:flex; flex-direction:column; align-items:center; gap:14px; width:100%; background:rgba(15, 23, 42, 0.6); border:1px solid rgba(255, 255, 255, 0.1); border-radius:16px; padding:20px; box-sizing:border-box;">
            <div style="display:flex; align-items:center; gap:12px; width:100%;">
              ${currentUser.user_metadata?.avatar_url || currentUser.user_metadata?.picture ? `<img src="${currentUser.user_metadata.avatar_url || currentUser.user_metadata.picture}" referrerpolicy="no-referrer" style="width:42px; height:42px; border-radius:50%; border:2px solid #7C3AED;" />` : `<div style="width:42px; height:42px; border-radius:50%; background:#7C3AED; color:#FFF; display:flex; align-items:center; justify-content:center; font-weight:700;">${(currentUser.user_metadata?.full_name || currentUser.email || 'U').charAt(0).toUpperCase()}</div>`}
