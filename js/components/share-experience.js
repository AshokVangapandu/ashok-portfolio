 ...
 * Testimonial text input with live counter.
 */
  ...
  connectedCallback() {
    ...
    this.innerHTML = `
      <div class="textarea-wrapper">
        <textarea id="testimonial-text" name="testimonial" placeholder="Ashok was incredible to work with because..." maxlength="${maxChars}" required ${isDisabled ? 'disabled' : ''}></textarea>
        <span class="char-counter"><span id="current-char-count">0</span> / ${maxChars}</span>
      </div>
    `;

    const textarea = this.querySelector("textarea");
    const counter = this.querySelector("#current-char-count");
...
class ShareExperienceModal extends HTMLElement {
  ...
  render() {
    ...
    const isFormDisabled = !this.userState.isAuthenticated || this.isLoading;

    this.innerHTML = `
      <button type="button" class="close-modal-btn" aria-label="Close modal" ${this.isLoading ? 'disabled' : ''}>
      ...
      <div class="auth-section" id="auth-section">
        <!-- Will dynamically render based on authentication state -->
      </div>
      ...
      <form class="share-form" id="share-experience-form" novalidate>
        ...
        <div class="form-group">
          ...
          <textarea-field id="form-testimonial" ${isFormDisabled ? 'disabled-textarea="true"' : ''}></textarea-field>
          <span class="field-error-msg" id="testimonial-error"></span>
        </div>

        <div class="form-group checkbox-group">
          <label class="checkbox-label">
            <input type="checkbox" id="consent-check" name="consent" required ${isFormDisabled ? 'disabled' : ''}>
        ...
        <button type="submit" class="profile-action profile-action-primary submit-form-btn magnetic" ${isFormDisabled ? 'disabled' : ''}>
  ...
  renderAuthState() {
    ...
    if (this.userState.isAuthenticated) {
      const avatarHtml = this.userState.avatar 
        ? `<img src="${this.userState.avatar}" alt="${this.userState.name}" class="user-avatar" />`
        : `<div class="user-avatar user-avatar-fallback">${this.userState.name.substring(0, 2).toUpperCase()}</div>`;

      authSection.innerHTML = `
        <div class="auth-user-badge">
          ${avatarHtml}
          <div class="user-info">
            <span class="user-name">${this.userState.name}</span>
            <span class="user-email">${this.userState.email}</span>
          </div>
          <button type="button" class="auth-logout-btn" id="auth-logout-btn" ${this.isLoading ? 'disabled' : ''}>Sign Out</button>
        </div>
      `;

      this.querySelector("#auth-logout-btn")?.addEventListener("click", () => {
        this.handleSignOut();
      });
    } else {
      authSection.innerHTML = `
        <div class="auth-prompt">
          ...
          <button type="button" class="google-auth-btn" id="google-auth-btn" ${this.isLoading ? 'disabled' : ''}>
            ${this.isLoading ? `
  ...
  setupFormHandlers() {
    ...
    form.addEventListener("submit", (event) => {
      ...
      if (!testimonialVal) {
        ...
        hasError = true;
      } else if (testimonialVal.length > 500) {
        this.showError("testimonial", "Your testimonial cannot exceed 500 characters.");
        hasError = true;
      }
