...
export const TechnologyGrid: React.FC = () => {
  ...
  const speedRef = useRef(0.88); // Increased speed by 20% more
  ...
  const updateCardStyles = () => {
    ...
    children.forEach((child) => {
      ...
      // Reduced Blur by 20%: max blur is 1.2px (was 1.5px)
          ...
          {tripledTechs.map((tech, idx) => {
            return (
              <React.Fragment key={idx}>
                ...
                {/* Diamond Separator (rendered after every card to make sets 100% mathematically identical) */}
                <span
                  style={{
      ...
      >
        <div
          style={{
            position: 'absolute',
            top: 0,
            left: `${scrollProgress * 70}%`, // thumb is 30% wide, so range is 0% to 70%
            width: '30%',
            height: '100%',
